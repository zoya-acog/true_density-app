...
//                    Enter multiple SMILES along with their compound IDs (new line separated).
//                   </Label>
//                   <Textarea
...
A, CC(C(=O)O)c1ccc(cc1)C(=O)C`;
  ...
  const singleSmilesSample = "c1ccccc1";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "An unexpected network error occurred.";
      setResults([{ smiles: "", compound: "Error" }]);
    } finally {
      setIsLoading(false);
    }
  };
