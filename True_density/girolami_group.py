...
def predict_density(smiles, compound="N/A"):
...
if __name__ == "__main__":
    try:
        with open("molecules.csv", newline='') as csvfile:
            ...
            results = []
            for row in reader:
                name = row.get("compound", "N/A")
                smiles = row["smiles"]
                try:
                    result = predict_density(smiles, name)
                    results.append(result)
                except Exception as e:
                    results.append({"compound": name, "smiles": smiles, "error": str(e)})
            print(json.dumps(results))
    except FileNotFoundError:
        print(json.dumps([{"error": "molecules.csv file not found"}]))
    except Exception as e:
